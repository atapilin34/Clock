  ...
  function setAlarm() {
	...
	const dateTimeSet = document.querySelector('#dateTimeSetInput');
	btn.addEventListener('click',(e)=>{
		dateTimeSet.style.display = 'block';
		
		
		
		
	});
	dateTimeSet.addEventListener('input',(e)=>{
		let dateTime = document.querySelector('[data-time]').value;
		document.querySelector('#dateTimeSetInput').disabled=true;
		let newInterval= setInterval(()=>{
			if ((Date.parse(new Date())-Date.parse(dateTime)) === 0){
				alertRGB();
				document.getElementById('budi').play();
			}			
		}, 500);
...
function alertRGB() {
	var colors = 'red-orange-yellow-green-aqua-black-violet'.split ('-');
	...
	document.addEventListener('keydown',(e)=>{
		...
		if (e.code === 'Escape') {
			...
			document.querySelector('#dateTimeSetInput').disabled=false;
			//clearInterval(newInterval);
			document.getElementById('budi').pause();
		}
	});
